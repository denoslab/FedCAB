...
class BaseTrainer(object):
    ...
    def local_train(self, round_i, selected_clients, strategy, baseline_param, **kwargs):
        ...
        for i, c in enumerate(selected_clients, start=1):
            ...
            # using old global model
            # if not (c.cid == 8 or c.cid == 9):
            #     # c.set_flat_model_params(self.latest_model)
            #     if self.cache_model_valid[round_i-25]:
            #         c.set_flat_model_params(self.cache_model[round_i-25])
            #     else:
            #         c.set_flat_model_params(self.latest_model)
            # else:
            #     c.set_flat_model_params(self.latest_model)

            # if c.cid == 0 or c.cid == 1:
            #     c.set_flat_model_params(self.diffed_model)

            # Solve minimization locally
            soln, stat = c.local_train(strategy, self.cache_model[round_i-2], baseline_param)
