...
def gen_avail_prob_equal(client_names, est_prob, max_prob, output_file, class_info):
    """
        For each client, adversarially determine the participation probability from min_prob to max_prob,
             based on the two classes that the client contains.
        For 10-class classification problem, the labels are indexed with 0 ~ 9.
...
def gen_avail_prob_varied(client_names, est_prob, max_prob, output_file, class_info):
    """
        For each client, adversarially determine the participation probability from min_prob to max_prob,
             based on the two classes that the client contains.
        For 10-class classification problem, the labels are indexed with 0 ~ 9.
...
def gen_avail_prob_reverse(client_names, est_prob, max_prob, output_file, class_info):
    """
        For each client, adversarially determine the participation probability from min_prob to max_prob,
             based on the two classes that the client contains.
        For 10-class classification problem, the labels are indexed with 0 ~ 9.
