...
def beautiful_print_report(report_dict):
    for algo, res in report_dict.items():
        gap = res['acc'] - report_dict[algo]['acc']
...
for options, logs in log.items():
    ...
    handler[alg_name] = plt.plot(time_steps, y_mean , color, linewidth=1.2)[0]  # , '--'
...
if target == 'train_acc':
    plt.xlabel("communication round", myfontdict2)
    plt.ylabel("training acc (%)", myfontdict2)
    plt.yscale('linear')
    if dataset == 'mnist':
        plt.ylim(0, 60)
    else:
...
if target == 'test_loss':
    plt.xlabel("communication round", myfontdict2)
    plt.ylabel("test loss", myfontdict2)
    plt.yscale('log')
    # scaling for MNIST
    if dataset == 'mnist':
        plt.ylim(0, 2.5)
...
if target=='test_acc':
    ...
    ax.tick_params(axis='both', which='minor', labelsize=20)
    plt.legend(handler_content, label, prop=myfontdict2)
    plt.savefig("./plot/{}_{}.png".format(dataset, avail_plots[int(sys.argv[2])]), bbox_inches='tight')
