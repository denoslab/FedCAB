...
def main():
    ...
    data.generate_equal.initMifaClientData(num_clients=options['num_user'])
    ...
    avail_prob_file_custom = os.path.join('./data', dataset_name + options['dirichlet'], 'data',
                                       'avail_prob_2_adversarial.pkl')

    # `dataset` is a tuple like (cids, groups, train_data, test_data)
    all_data_info = read_data(train_path, test_path, avail_prob_file_custom, sub_data)
